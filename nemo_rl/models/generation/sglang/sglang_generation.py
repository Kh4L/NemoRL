...
class SGLangGeneration(GenerationInterface):
    ...
    def __getstate__(self):
        # Shipped to the AsyncTrajectoryCollector Ray actor: drop transient,
        # non-picklable runtime (event-loop thread, http client, health monitor).
        # Ray actor handles (engines/pg/lock) pickle fine and are kept.
    ...
    def __setstate__(self, state):
        ...
        # Re-create transient runtime on the receiving process.
        self._async_loop = AsyncLoopThread()
        self._http_client = init_http_client(self.sglang_cfg)
    ...
    def __init__(
        ...
        self.cfg = sglang_cfg  # vLLM-compatible alias read by grpo/rollouts/nemo_gym
        self._health_monitor = None
        ...
        # B2: per-engine SGLang OpenAI base URLs (/v1) for the NeMo-Gym proxy (NOT the router).
        self.dp_openai_server_base_urls = [
            f"{u}/v1"
            for u in ray.get([e.get_base_url.remote() for e in self.engines])
            if u is not None
        ]
    ...
    def post_process_weights(
        ...
        *,
        ...
        Called by the Megatron-side refit dispatch helpers after a colocate
        IPC or distributed broadcast refit so SGLang finalizes its weight
        tables (e.g. materializes quantized scales, swaps in the freshly
        loaded buffer).
        """
        engines = [e for e in self.engines if e is not None]
        if not engines:
